...
describe("Python import-bound reference precision (T1-B)", () => {
  it("meets the \u22650.90 precision / \u22650.70 recall target on the Python fixture", async () => {
    const report = await measurePrecision(join(fixtures, "python"));
    if (report.totals.precision < 0.9 || report.totals.recall < 0.7) {
      // eslint-disable-next-line no-console
      console.error(
        "Python precision report:\n" + JSON.stringify(report, null, 2),
      );
    }
    expect(report.totals.precision).toBeGreaterThanOrEqual(0.9);
    expect(report.totals.recall).toBeGreaterThanOrEqual(0.7);
  });
});
